...
object FirebaseDatabaseHelper {
    private val database =
        FirebaseDatabase.getInstance("https://sameway-3bec4-default-rtdb.europe-west1.firebasedatabase.app/")
    ...
    fun requestAccessFromDriver(route: Route, start: LatLng, end: LatLng) {
                ...
                val routes = snapshot.children.mapNotNull { userSnapshot ->
                    ...
                    if (routeSnapshot.exists()) {
                        ...
                        Route(
                            ...
                            status = status,
                            clients = clients.map {Client(user = it.user, start = it.start.toLatLng(), end = it.end.toLatLng(), status = it.status)},
                        )
                    } else {
                        null
                    }
        ...
        usersRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                val clients = snapshot.child(uid).child("route").child("clients").children.mapNotNull { clientSnapshot ->
                    ...
                    Client(user = userId, start = start.toLatLng(), end = end.toLatLng(), status = status)
                }
                callback(clients)
            }
